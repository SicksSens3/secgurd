...
- **Per-user PUP / clone-browser detection** — walks every user's AppData (Local/Roaming/LocalLow), **the profile root itself**, and every user registry hive — including **logged-off users' `NTUSER.DAT`**, which it mounts then unloads — to catch adware / potentially-unwanted apps that install per-user and skip Add/Remove Programs. Flags the Chromium-"clone" layout and updater/dock families (`<App>` + `<App>Updater`/`AutoUpdate`/`Dock`), plus self-registered `Software\<Name>` keys carrying an `UninstallString`/`InstallerProgress` value. Catches infections sitting in **other** users' profiles that a current-user-only scan misses (mounting logged-off hives needs admin). Findings and the `09_user_hive_software.txt` rows cite the **full `HKU\<SID>\…` registry path** — built from the profile's *real* SID, even for a mounted logged-off hive (whose live key is a temporary mount point) — so a flagged key is directly navigable. Module 02 carries a **SID→account map** (`Get-CimInstance Win32_UserProfile`, covering **domain/AD** profiles that `Get-LocalUser` never lists) so any SID seen here — local or domain — can be decoded back to a user.
- **Profile-root installs are no longer a blind spot** — every "writable path" test in the tool used to ask one question: is this under `Temp`, `AppData`, `Users\Public` or `ProgramData`? An installer-style PUP needs none of them. It drops into the **profile root** — `%USERPROFILE%\<Vendor>\` — which needs no admin rights either and was on no list. Wave Browser is the textbook case: it installs to `C:\Users\<u>\Wavesor Software\WaveBrowser\wavebrowser.exe` and persists with a task under `\Wavesor Software_<SID>\WaveBrowser-StartAtLogin`, so its **task action, its `Run` value, its service path and its install folder all scored clean** — not AppData, not Temp, therefore never examined — and the per-user PUP walk never saw the folder because it only descended AppData. Profile-root installs are now judged as their own reason (`profile-root-install`) across scheduled tasks, service paths, per-user `Run`/`RunOnce` entries and the install-folder walk, with the folders Windows, a sync client (`OneDrive - <Tenant>`, Dropbox, Box, iCloud) or a dev toolchain (`scoop`, `go`, `anaconda3`, `.cargo`, `source`/`repos`, …) legitimately create there exempted by name. The Chromium-clone check is now **structural rather than name-based** — an `.exe` sitting beside a bare-version folder — so it also catches clones laid out as `<Vendor>\<Product>\{<product>.exe, 116.0.5845.111\}` instead of the standard `<App>\Application\<ver>\`, which is precisely the layout that walked past it.
- **Per-user task folders carrying a SID** — a scheduled-task folder named `\<Vendor>_S-1-5-21-…\` is how a per-user installer scopes persistence to one account: no admin needed, and a copy survives in every profile the installer ran in. Legitimate software registers under `\<Vendor>\` or `\Microsoft\…`; the SID suffix is close to exclusive to bundleware and clone-browser installers. Each one is flagged with the **vendor, the task, and the account the SID resolves to** — which is the answer to "*which* user is infected", something a task name alone never tells you.
- **Named PUP / adware / browser-hijacker families** (`09_pup_adware.txt`) — the collectors above detect the *shape* of a per-user install; this one supplies the **name**. Wave Browser / Wavesor, OneLaunch, WebDiscover, MediaGet, Segurazo/SAntivirus, TotalAV/PCProtect/ScanGuard, driver-updater and system-repair PUPs, toolbar/search hijackers and bundleware installers are matched across **install folders (profile root, AppData, Program Files, ProgramData), scheduled tasks (name, folder path *and* action), services, running processes, and the registry** — Add/Remove entries, self-registered `Software` keys, `Run`/`RunOnce` values, and the `StartMenuInternet` client registration a clone browser needs in order to be offered as a real browser (and which is routinely left behind by an uninstall). Unlike the RMM table, none of these are legitimate enterprise software, so a match is reported without asking whether it was authorized — the tool being present *is* the finding. All evidence for one product folds into **one HIGH finding** with an itemised detail block, rather than eight near-identical ones, and the artifact closes with the remediation order that actually works: persistence first, folder second.
- **Hijacker browsers get read like browsers** — a clone browser keeps a Chromium `User Data` tree of its own, and it is the one that matters: the point of installing Wave Browser on somebody is to route their searches through it. Its history, extensions and notification grants are now collected alongside Chrome/Edge/Brave, instead of the scan answering "what did they browse" only for the browsers the PUP was trying to replace.
...
**The module menu draws whenever there's a keyboard, not only in a redirected shell.** The `redirected stdin AND non-interactive` combo that flags the S1 remote shell is *identical* to the combo a genuinely dead pipe produces — the two can't be told apart from those flags. The module menu used to render only when that combo was true, which meant a plain interactive console — `.\secgurd.ps1` run locally, or the `iex (irm …)` web launcher pasted into a normal PowerShell window, where stdin is **not** redirected — printed the banner and fell straight through to `return`, collecting nothing and looking like the fetch had failed. The menu now always draws (a console and the S1 shell both drive `Read-Host` fine); the flag combo is kept only as a hint for the prompt's end-of-stream fallback, so a *truly* piped run (where `Read-Host` hits EOF and returns `$null`, which a bare Enter never does) selects every module and scans instead of exiting silently. Headless callers still use `-Auto` / `-Modules`, which never reach the menu.
...
secgurd_<HOST>_<timestamp>\
  ...
  09_appdata_app_installs.txt     per-user app installs - AppData AND profile root (PUP / clone-browser flags)
  09_user_hive_software.txt       self-registered software across all user hives (incl. logged-off)
  09_pup_adware.txt               named PUP / adware / browser-hijacker families, and where each one persists
